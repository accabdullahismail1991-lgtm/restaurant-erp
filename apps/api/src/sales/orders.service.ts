...
export class OrdersService {
  ...
  async create(dto: CreateOrderDto, userId: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      //
      // ANY ingredient a menu item's recipe needs -- SEMI_FINISHED or
      // RAW_MATERIAL, with or without a registered production recipe of
      // its own -- is treated as sellable past zero EVEN WHEN
      // allowNegativeStock is off, as long as autoGenerateProductionOrders
      // is on: its shortage becomes a visible restock/production need on
      // the Production Orders screen (see ProductionOrdersService.
      // applyShortfall, which falls back to an inputs-less "go
      // restock/buy N more" order when there's no BOM to base real inputs
      // on) rather than silently blocking the sale.
      //
      ...
      const consumeRecipeFor = async (menuItemId: string, lineQty: number) => {
        ...
        for (const recipeLine of recipeLines) {
          const allowNegative = location.allowNegativeStock || location.autoGenerateProductionOrders;
