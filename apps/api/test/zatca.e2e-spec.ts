...
describe('Phase 9: ZATCA invoice generation (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.role.deleteMany({ where: { name: 'Zatca-Test-Shift' } });
    ...
    const shiftRole = await prisma.role.create({ data: { name: 'Zatca-Test-Shift' } });
    await prisma.rolePermission.create({ data: { roleId: shiftRole.id, permissionId: shiftPerm.id } });

    const passwordHash = await bcrypt.hash(PASSWORD, 10);
    const adminUser = await prisma.user.create({ data: { name: 'Zatca Admin', phone: ADMIN_PHONE, passwordHash } });
