...
describe('Phase 10c: customers / loyalty (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.role.deleteMany({ where: { name: 'Customers-Test-Shift' } });

    // adminToken opens a shift as scaffolding for the paid-order tests below.
    ...
    const shiftRole = await prisma.role.create({ data: { name: 'Customers-Test-Shift' } });
    await prisma.rolePermission.create({ data: { roleId: shiftRole.id, permissionId: shiftPerm.id } });

    const passwordHash = await bcrypt.hash(PASSWORD, 10);
    const adminUser = await prisma.user.create({ data: { name: 'Customer Admin', phone: ADMIN_PHONE, passwordHash } });
