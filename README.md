...
- المتبقي: تكامل ZATCA (توليد/توقيع الفاتورة فعليًا)، KDS/Multi-channel/Promotions/CRM،
...
1. كل الوحدات التشغيلية الأساسية (المراحل 1-7 + الجرد) مبنية ومُختبرة الآن، وكذلك واجهة كاشير
   ...
   التالي في `docs/ARCHITECTURE.md` → قسم "خطة التنفيذ المرحلية": **المرحلة 9 (ZATCA Phase 2)**
   -- توليد وتوقيع الفاتورة الضريبية فعليًا وربطها بمنصة فاتورة، بما فيها القرار المفتوح في
   `docs/ARCHITECTURE.md` عن التوليد/التوقيع أثناء وضع عدم الاتصال (نفس طابور المزامنة الموجود
   الآن في `apps/pos-web/src/offline/`).
