...
- المتبقي من المرحلة 10: CRM/Loyalty (نقاط ولاء)، ثم الرفع الفعلي لـ ZATCA (بمجرد توفر بيانات
  اعتماد حقيقية)، ثم BI/Analytics متقدم — راجع `docs/ARCHITECTURE.md` لتفاصيل كل مرحلة.
...
1. كل الوحدات التشغيلية الأساسية (المراحل 1-7 + الجرد) مبنية ومُختبرة الآن، وكذلك واجهة كاشير
   ...
   `ZatcaService.submitToZatca` (التعليق هناك يشرح بالضبط الثلاث خطوات اللازمة). المرحلتان 10a
   (KDS، `src/kitchen/`) و10b (Promotions، `src/promotions/`) مكتملتان أيضًا. التالي في
   `docs/ARCHITECTURE.md` → قسم "خطة التنفيذ المرحلية": **المرحلة 10c (CRM/Loyalty)** -- نقاط
   ولاء بسجل حركات (Ledger) لا عدّاد خام (`Customer.points` موجود في المخطط، القرار #15، لكن
   بلا سجل تدقيق ولا منطق كسب/استبدال بعد).
...
│   │   └── test/*.e2e-spec.ts                       ← اختبارات كل المراحل المبنية (119 اختبار)
...
│   └── admin_panel.html        ← لوحة تحكم تغطي المراحل 1-7 + 10b (فروع/أصناف/مخزون/مبيعات/مشتريات/إنتاج/تحويلات/عروض)
└── docker-compose.yml
```
