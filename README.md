...
- ⬜ لم يُبنَ بعد: بقية الوحدات (`purchasing`, `production`, `transfers`)، Frontend فعلي،
  آلية المزامنة Offline-first، تكامل ZATCA (توليد/توقيع الفاتورة فعليًا)، تكامل الدفع
...
1. المرحلة التالية بالترتيب المقترح في `docs/ARCHITECTURE.md` → قسم "خطة التنفيذ المرحلية":
   **المرحلة 5: Purchasing + Approval Matrix** — أوامر شراء (PO) كاملة مع مسار موافقة (جدول
   `ApprovalRule` منفصل غير مبرمج بالكود، القرار #8)، تنتهي باستلام يُنشئ `InventoryBatch` حقيقية
   عبر `InventoryService.receive` الموجودة بالفعل (لا تعيد بناءها).
2. لا تبدأ بكل الوحدات دفعة وحدة — كل وحدة يجب أن تُبنى وتُختبر (باختبارات e2e حقيقية، مو Unit
   tests وهمية فقط) قبل الانتقال للتالية، بنفس نمط المراحل 1-4.
...
4. `InventoryService` (استلام/خصم FIFO/عكس استهلاك) و`src/common/location-scope.util.ts` مبنيان
   كوحدات مشتركة يُعاد استخدامها -- لا تكرّر منطقهما في وحدات المشتريات/الإنتاج/التحويلات القادمة.
...
├── apps/
│   ├── api/                  ← Backend (NestJS) -- المراحل 1-4 مبنية وتعمل
...
│   │   └── test/*.e2e-spec.ts               ← اختبارات المراحل 1-4 الكاملة (37 اختبار)
