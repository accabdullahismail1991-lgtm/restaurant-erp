...
- المتبقي: الرفع الفعلي لـ ZATCA (بمجرد توفر بيانات اعتماد حقيقية)، ثم BI/Analytics متقدم —
  راجع `docs/ARCHITECTURE.md` لتفاصيل كل مرحلة.
...
1. كل الوحدات التشغيلية الأساسية (المراحل 1-7 + الجرد) مبنية ومُختبرة الآن، وكذلك واجهة كاشير
   ...
   `ZatcaService.submitToZatca` (التعليق هناك يشرح بالضبط الثلاث خطوات اللازمة). **المرحلة 10
   اكتملت بالكامل الآن** (10a KDS `src/kitchen/`، 10b Promotions `src/promotions/`، 10c
   CRM/Loyalty `src/customers/`). التالي في `docs/ARCHITECTURE.md` → قسم "خطة التنفيذ المرحلية":
   **المرحلة 11 (BI/Analytics متقدم)** -- طبقة تحليلية منفصلة تُبنى فوق نفس الجداول التشغيلية
   دون تعديلها (القرار #20).
...
│   │   └── test/*.e2e-spec.ts                        ← اختبارات كل المراحل المبنية (127 اختبار)
...
│   └── admin_panel.html        ← لوحة تحكم تغطي المراحل 1-7 + 10b + 10c (فروع/أصناف/مخزون/مبيعات/مشتريات/إنتاج/تحويلات/عروض/عملاء)
└── docker-compose.yml
```
