...
- ⬜ لم يُبنَ بعد: `transfers`، Frontend فعلي، آلية المزامنة Offline-first، تكامل ZATCA
  (توليد/توقيع الفاتورة فعليًا)، تكامل الدفع
...
1. المرحلة التالية بالترتيب المقترح في `docs/ARCHITECTURE.md` → قسم "خطة التنفيذ المرحلية":
   **المرحلة 7: Transfers** — تحويل بين موقعين بحالتين (Dispatched/Received) والفرق بين الكمية
   المُرسَلة والمُستلَمة = فاقد نقل تلقائي (القرار #6)؛ الإرسال يخصم من موقع المصدر عبر
   `InventoryService.consume`، والاستلام يضيف لموقع الوجهة عبر `InventoryService.receive` بنفس
   منطق المشتريات/الإنتاج تمامًا.
2. لا تبدأ بكل الوحدات دفعة وحدة — كل وحدة يجب أن تُبنى وتُختبر (باختبارات e2e حقيقية، مو Unit
   tests وهمية فقط) قبل الانتقال للتالية، بنفس نمط المراحل 1-6.
...
4. `InventoryService` (استلام/خصم FIFO/عكس استهلاك) و`src/common/location-scope.util.ts` مبنيان
   كوحدات مشتركة يُعاد استخدامها -- لا تكرّر منطقهما في وحدة التحويلات القادمة.
5. **لأي اختبار e2e جديد**: استخدم `test/reset-db.ts`'s `resetDatabase(prisma)` في بداية
   ...
   هيكسر تنظيف ملف اختبار تاني. **أضف جداول التحويلات الجديدة لنفس الدالة** (بترتيب children قبل
   parents) بدل تكرار المنطق في كل ملف اختبار كما كان يحصل قبل هذا الريفاكتور.
...
├── apps/
│   ├── api/                  ← Backend (NestJS) -- المراحل 1-6 مبنية وتعمل
...
│   │   └── test/*.e2e-spec.ts                  ← اختبارات المراحل 1-6 الكاملة (72 اختبار)
