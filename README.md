...
- المتبقي من المرحلة 10: Promotions (محرك عروض) وCRM/Loyalty (نقاط ولاء)، ثم الرفع الفعلي لـ
  ZATCA (بمجرد توفر بيانات اعتماد حقيقية)، ثم BI/Analytics متقدم — راجع `docs/ARCHITECTURE.md`
  لتفاصيل كل مرحلة.
...
1. كل الوحدات التشغيلية الأساسية (المراحل 1-7 + الجرد) مبنية ومُختبرة الآن، وكذلك واجهة كاشير
   ...
   `ZatcaService.submitToZatca` (التعليق هناك يشرح بالضبط الثلاث خطوات اللازمة). المرحلة 10a
   (KDS) مكتملة أيضًا (`src/kitchen/`). التالي في `docs/ARCHITECTURE.md` → قسم "خطة التنفيذ
   المرحلية": **المرحلة 10b (Promotions)** -- محرك عروض (نسبة/مبلغ ثابت) كطبقة حساب منفصلة على
   الفاتورة (`Promotion` موجود في المخطط، القرار #14، لكن غير مُفعَّل بعد في منطق الطلبات).
...
│   │   └── test/*.e2e-spec.ts                      ← اختبارات كل المراحل المبنية (109 اختبار)
